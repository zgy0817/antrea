...
func (pt *PortTable) DeleteRule(podIP string, podPort int, protocol string) error {
	...
	pt.delPortTableCacheFromNodePortIndex(NodePortProtoFormat(data.NodePort, protocol))
	return nil
}
...
func (pt *PortTable) DeleteRulesForPod(podIP string) error {
	...
	for _, podEntry := range podEntries {
		...
		pt.delPortTableCacheFromNodePortIndex(NodePortProtoFormat(podEntry.NodePort, protocolSocketData.Protocol))
	}
	return nil
}
