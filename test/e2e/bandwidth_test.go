...
func benchmarkBandwidthServicePodAccess(t *testing.T, data *TestData, endpointNode, clientNode string, access string) {
	...
	defer data.deletePodAndWait(defaultTimeout, server)
	require.NoErrorf(t, NewPodBuilder(server, testNamespace, perftoolImageUsed).OnNode(clientNode).WithPorts([]v1.ContainerPort{{Protocol: v1.ProtocolTCP, ContainerPort: iperfPort}}).Create(data), "Error when creating the perftest server Pod")
	...
	require.NoErrorf(t, NewPodBuilder(client, testNamespace, perftoolImageUsed).OnNode(clientNode).Create(data)), "Error when creating the perftest client Pod")
...
func benchmarkBandwidthServiceNodeAccess(t *testing.T, data *TestData, endpointNode, clientNode string, access string) {
	...
	require.NoErrorf(t, NewPodBuilder(server, testNamespace, perftoolImageUsed).OnNode(endpointNode).WithPorts([]v1.ContainerPort{{Protocol: v1.ProtocolTCP, ContainerPort: iperfPort}}).Create(data), "Error when creating the perftest server Pod")
	defer data.deletePodAndWait(defaultTimeout, server, testNamespace)
	require.NoErrorf(t, data.podWaitForRunning(defaultTimeout, server, testNamespace), "Error when getting the perftest server Pod's IP")
...
func testServiceNodePortPodAccess(t *testing.T, data *TestData) {
	...
	require.NoErrorf(t, err, "Error when creating perftest service")
	defer data.deleteServiceAndWait(defaultTimeout, server)
	require.NoErrorf(t, NewPodBuilder(server, testNamespace, perftoolImage).OnNode(endpointNode).WithPorts([]v1.ContainerPort{{Protocol: v1.ProtocolTCP, ContainerPort: iperfPort}}).Create(data), "Error when creating the perftest server Pod")
	defer data.deletePodAndWait(defaultTimeout, server, testNamespace)
	require.NoErrorf(t, data.podWaitForRunning(defaultTimeout, server, testNamespace), "Error when getting the perftest server Pod's IP")
	var nodePort string
	...
	require.NoErrorf(t, NewPodBuilder(client, testNamespace, perftoolImage).OnNode(clientPod).Create(data), "Error when creating the perftest client Pod")
	defer data.deletePodAndWait(defaultTimeout, client, testNamespace)
	require.NoErrorf(t, data.podWaitForRunning(defaultTimeout, client, testNamespace), "Error when waiting for the perftest client Pod")
